...
function countVowels(str) {
  let res = 0;
  const volw = ['a', 'e', 'i', 'o', 'u', 'y', 'A', 'E', 'I', 'O', 'U', 'Y'];
  for (let i = 0; i < str.length; i += 1) {
    if (volw.includes(str[i])) {
      res += 1;
    }
  }
  return res;
...
function isPalindrome(str) {
  let res = false;
  if (
    str.toLowerCase().replace(/[^\w]/g, '').split('').join('') ===
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < sentence.split(' ').length; i += 1) {
    if (sentence.split(' ')[i].length > longWord.length) {
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (
      ...
      res += String.fromCodePoint(str[i].charCodeAt(0) + 13);
    } else if (
